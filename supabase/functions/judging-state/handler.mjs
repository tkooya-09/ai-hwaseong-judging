export function createHandler(C, env, request = fetch) {
  ...
  return async function handler(req) {
    try {
      ...
      if(token.length<24||token.length>128)return json({error:'접속 코드가 일치하지 않습니다.'},401);
      ...
      const credentials=await db('hwaseong_judging_codes?token_hash=eq.'+hash+'&active=eq.true&select=role,event_id&limit=1');
      if(!credentials.length)return json({error:'접속 코드가 일치하지 않습니다.'},401);
